...
function subscribe(publicAddress) {
	...
	chain.createNotification({type: "address", block_chain: configObject["chain"].useTestnet ? "testnet3" : "bitcoin" , address: publicAddress, url:  configObject["webserver"] + "/notifications"}, function(err, resp) {});

}

function subscribeForOrganizationNotifications(publicAddress) {

	chain.createNotification({type: "address", block_chain: configObject["chain"].useTestnet ? "testnet3" : "bitcoin" , address: publicAddress, url: configObject["webserver"] + "/organization/notifications"}, function(err, resp) {});
